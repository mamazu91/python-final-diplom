            ...
            try:
                ...
                if not price_list:
                    raise ValidationError(
                        {'results': [f"Could not parse price list. Error: file '{price_list_filepath} is empty."]})
                if not is_price_list_valid(price_list):
                    raise ValidationError(
                        {'results': ['Could not parse price list. Error: price list is of invalid format.']})
                return price_list
            except yaml.YAMLError as yaml_load_exception:
                yaml_load_error = yaml_load_exception.__dict__.get('problem')
                raise ValidationError(
                    {'results': [f'Could not load price list. Error: {yaml_load_error}']} if yaml_load_error
                    else {'results': ['Unknown error.']})
    except FileNotFoundError:
        raise ValidationError(
            {'results': [f"Could not load price list. Error: file '{price_list_filepath}' does not exist."]})
