...
#     supplier = client_factory(is_supplier=True, is_confirmed=True)
#     client_token = Token.objects.create(user=supplier)
...
#     assert sorted([shop.name for shop in supplier_shops]) == sorted(
#         [shop.get('name') for shop in response.data])
