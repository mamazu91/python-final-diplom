...
def test_open_shops_list(api_client, shop_factory):
...
@pytest.mark.django_db
def test_categories_list(api_client, category_factory):
...
def test_product_detail(api_client, product_factory):
    ...
    # Intention here is to check that the product will have position_id field in every shop it is available in,
...
def test_products_list_filter_category(api_client, category_factory, product_factory):
    ...
    assert response.status_code == 200
    assert sorted([product.name for product in products]) == sorted([product.get('name') for product in response.data])
